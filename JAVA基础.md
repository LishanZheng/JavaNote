...
1. 互斥条件：资源任一时刻只能有一个线程占用。
2. 请求与保持条件：占有部分资源的线程请求新资源被阻塞后，之前占有的资源仍然占有。
3. 不剥夺条件：还没运行完毕的线程，已经持有的资源在没有运行完毕之前不会被剥夺。
4. 循环等待条件：若干个线程之间形成一种头尾相连的循环等待资源情况。

死锁的预防，破坏死锁产生条件

1. 破坏请求与保持条件：一次性请求所有资源。
2. 破坏不剥夺条件：当占有其他资源的线程请求新的资源没有被满足时，就释放之前占有的资源。

3. 破坏循环等待条件：按照某种顺序请求资源，释放资源的时候按照反序。
